...
fn pixel_ycbcr_to_rgb(ycbcr_val: &YCbCrVal) -> RGBVal {
    ...
    RGBVal {
        red: (f_luminance + 1.402 * (f_c_red - 128.0)) as u8,
        green: (f_luminance - 0.344136 * (f_c_blue - 128.0) - 0.714136 * (f_c_red - 128.0)) as u8,
        blue: (f_luminance + 1.772 * (f_c_blue - 128.0)) as u8, 
    }
}
...
// Result vec should be difference between vec1 and vec2 - difference multiplied by 10
// Note - difference has a ceiling of 255, b/c 8-bit color
pub fn get_difference(vec1: &mut Vec<u8>, vec2: &mut Vec<u8>, res_vec: &mut Vec<u8>, ampl_factor: u32) {
    ...
    while i < vec1.len() {
        ...
        res_vec.push((min::<u32>(ampl_factor * res as u32, 255)) as u8);
        i += 1;
    }
}